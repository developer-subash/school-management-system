...

{
    ...
    // Final Marksheet for student

    function final_marksheet()
    {
        ...
        $page_data['page_name']  = 'final_marsheet';
        $page_data['page_title'] = get_phrase('final_marksheet');
        $running_year =  $this->db->get_where('settings' , array(
                 'type' => 'running_year'))->row()->description;
        $page_data['exam'] = $this->db->get_where('exam',array('year' => $running_year))->result();
        $page_data['class'] = $this->db->get_where('class',array('year' => $running_year))->result();
	...
	function student_bulk_add($param1 = '')
	{
        ...
        if($param1 == 'add_bulk_student') {
            ...
            for($i = 0; $i < $student_entries; $i++) {
                ...
                $data['phone']    =   $phones[$i];
                $data['permanent_address']  = $addresses[$i];
                $data['sex']      =   $genders[$i];
    ...
    function eaa_category($param1 ='')
    {

            if($param1 == 'add')
            {

                $page_data['category_name'] = $_POST['category_name'];

                    
                $this->db->insert('tbl_caa_category',$page_data);
                $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
            }

        $page_data['page_name']  = 'caa_category';
        $page_data['page_title'] = get_phrase('eaa_category');
        $page_data['running_year'] = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $page_data['class'] = $this->db->get_where('class',array('year' => $page_data['running_year']))->result();

        $this->load->view('backend/index', $page_data);   
    }

    function eaa_item($param1='')
    {   

        if($param1 == 'add')
        {

            $page_data['item_name'] = $_POST['item_name'];
            $page_data['year'] = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;

            $this->db->insert('tbl_caa_item',$page_data);
                $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));         

        }

        $page_data['page_name']  = 'caa_items';
        $page_data['page_title'] = get_phrase('caa_item');
        $page_data['running_year'] = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $page_data['class'] = $this->db->get_where('class',array('year' => $page_data['running_year']))->result();
        $page_data['caa_list'] = $this->db->get('tbl_caa_item')->result_array();
        $this->load->view('backend/index', $page_data);   

    }

    function store_eaa($param1='',$param2='')
    {
            ...
            if($param1 == 'add')
            {
                ...
                foreach ($_POST['caa_item_id']  as $key => $value) {

                        $page_data['caa_item_id'] = $value;
                        $page_data['class_id'] = $_POST['class_id'];
                        $page_data['comment'] = $_POST['comment'];
                        $page_data['caa_category_id'] = $_POST['caa_category_id'];
                        $page_data['year'] = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;

                        $this->db->insert('tbl_caa_store',$page_data);

                        $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));         
                }       
            }

        $page_data['page_name']  = 'caa_store_items';
        $page_data['page_title'] = get_phrase('caa_store_item');
        $page_data['running_year'] = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $page_data['class'] = $this->db->get_where('class',array('year' => $page_data['running_year']))->result_array();
        $page_data['caa_list'] = $this->db->get('tbl_caa_item')->result_array();
        $page_data['caa_category'] = $this->db->get('tbl_caa_category')->result();
        $page_data['caa_store_item'] = $this->db->get('tbl_caa_store')->result();     
    ...
    function create_case($param1='')
    {
        ...
        if($param1 == 'add')
        {
            ...
            $page_data['case_title_id'] = $_POST['case_title_id'];
            $page_data['exam_id'] = $_POST['exam-id'];
            
            $this->db->insert('tbl_case',$page_data);

            $this->session->set_flashdata('flash_message' , get_phrase('data_added_successfully'));
        }

        $page_data['page_name']  = 'store_case';
        $page_data['page_title'] = get_phrase('store_case');
        $page_data['running_year'] = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $page_data['class'] = $this->db->get_where('class',array('year' => $page_data['running_year']))->result_array();
        $page_data['exam'] = $this->db->get_where('exam',array('year' => $page_data['running_year']))->result();
    ...
    function get_case_item($exam_id)
    {
        ...
        $query = $this->db->get_where('exam',array('exam_id' => $exam_id))->row();

        echo '<option>'.get_phrase('Select_case_title').'</option>';

        echo '<option value='.$query->exam_id.'>'.$query->name.'</option>';
    ...
    function get_subject_marks()
    {
       ...
       // $exam_held_date = $_POST['exam_held_date'];
       $class_id = $_POST['class_id'];
       $subject_id = $_POST['subject_id'];

       $query = $this->db->select('*')->from('tbl_routine')->where('exam_id',$exam_id)->where('class_id',$class_id)->where('subject_id',$subject_id)->get()->row();

       echo '<option value="'.$query->max_num.'">'.$query->max_num.' </option>';    
    }

    function get_exam_held_date($exam_id)
    {

        $exam = $this->db->select('*')->from('exam')->where('exam_id',$exam_id)->get()->row();



        echo '<option value="'.$exam->start_date.'">'.date('Y M j',strtotime($exam->start_date)). ' ' .'To'.' ' .date('Y M j', strtotime($exam->end_date)) .' </option>';
    }

    function get_pass_marks()
    {
       ...
       echo '<option value="'.$query->min_num.'">'.$query->min_num.' </option>';
    ...
    function get_student_details()
    {
        ...
        // echo $running_year;exit();
        $enroll = $this->db->select('*')->from('enroll')->where('class_id',$class_id)->where('section_id',$section_id)->where('year',$running_year)->get()->result();
        
        foreach ($enroll as $key => $value) 
        {
            $student_name = $this->db->get_where('student',array('student_id'=>$value->student_id))->row()->name;
    ...
    function set_term_marks($param1 = '',$param2 = '')

    {

        $page_data['page_name']  =   'set_term_marks';
        $page_data['page_title'] =   get_phrase('set_term_marks');
        $page_data['running_year'] =   $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;

        

        $page_data['exam_term'] = $this->db->get_where('exam',array('year'=>$page_data['running_year']))->result_array();
    ...
    function add_term_marks()
    {
                ...
                $page_data['page_name']  =   'set_std_marks';
                $page_data['page_title'] =   get_phrase('set_term_marks'); 
                // $page_data['std_marks'] = $this->db->select('*')->from('tbl_term_marks')->where('class_id',$_POST['class_id'])->where('exam_id',$_POST['exam_id'])->get()->result_array();

                // print_r($page_data['std_marks']);exit();    
                $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
                $page_data['student_details'] = $this->db->select('*')->from('enroll')->where('class_id',$_POST['class_id'])->where('section_id',$_POST['section_id'])->where('year',$year)->get()->result();

                $page_data['subject'] = $this->db->select('*')->from('subject')->where('class_id',$_POST['class_id'])->where('year',$year)->get();
                $page_data['subjects'] = $this->db->select('*')->from('tbl_routine')->where('academic_year',$year)->where('exam_id',$_POST['exam_id'])->where('class_id',$_POST['class_id'])->get()->result();

                $page_data['students'] = $this->db->select('*')->from('enroll')->where('class_id',$_POST['class_id'])->where('section_id',$_POST['section_id'])->where('year',$year)->get()->result();    


                    $this->load->view('backend/index',$page_data);

    }
    function get_subash_details()
    {
        ...
        $query = $this->db->select('*')->from('subject')->where('class_id',$class_id)->where('year',$year)->get();
                ...
                foreach ($query->result() as $key => $value) {
                    ...
                    $html = '<div>    
                            ...
                            <td style="text-align: center;">
                              "'.get_phrase('students').'"  <i class="entypo-down-thin"></i> | "'.get_phrase('date').'"  <i class="entypo-right-thin"></i>
                        </td>
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if($this->input->post('blood_group') != null){
             ...
             if($this->input->post('current_address') != null){
              $data['current_address']      = $this->input->post('current_address');
            }
            if($this->input->post('permanent_address') != null){
            ...
            if($this->input->post('mother_tongue') != null){
              $data['mother_tongue']        = $this->input->post('mother_tongue');
            }
            if($this->input->post('cast_category') != null){
              $data['cast_category']        = $this->input->post('cast_category');
            }
            if($this->input->post('religion') != null){
              $data['religion']        = $this->input->post('religion');
            }
            if($this->input->post('disability') != null){
              $data['disability']        = $this->input->post('disability');
            }
            if($this->input->post('known_allergy') != null){
              $data['known_allergy']        = $this->input->post('known_allergy');
            }
            ...
            if($validation == 1){
                ...
                if ($this->input->post('roll') != '') {
                 ...
                 if ($this->input->post('joining_date') != '') {
                    $data2['joining_date']           = $this->input->post('joining_date');
                }
                $data2['date_added']     = strtotime(date("Y-m-d H:i:s"));
    ...
    /** store case items here for case title ***/

    function set_case_title($param1='')
    {
        ...
        if($param1 == 'add')
        {
         ...
         if($_SERVER['REQUEST_METHOD'] == "POST")
            ...
            $this->db->insert('tbl_case_title',$page_data);

            $this->session->set_flashdata('flash_message' , get_phrase('new_case_item_added'));


            } else {

                redirect(base_url() . 'index.php?admin/set_case_title/');
            }

        }

        $page_data['page_title']    = get_phrase('set_case_title');
        $page_data['page_name']  = 'set_case_title';
        $page_data['case_item'] = $this->db->get('tbl_case_title')->result();

        $this->load->view('backend/index', $page_data);   

    } 

    // store student case marks 

    function set_case_marks_view()

    {   
      
        $page_data['page_title']    = get_phrase('set_case_marks');
        $page_data['page_name']  = 'set_case_marks';
            
        $page_data['case_item'] = $this->db->get('tbl_case_title')->result();

         $year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
        $page_data['year'] = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
        $page_data['marks'] = $this->db->get('tbl_term_marks')->result();
        $page_data['class'] = $this->db->get('class')->result();
        $data['enroll'] = $this->db->select('*')->from('enroll')->where('year',$year)->get()->result();
    ...
    function store_cases_marks()
    {
        if($_SERVER['REQUEST_METHOD'] == "POST")
        {
           ...
           foreach ($_POST['item_id'] as $key => $value) {
            ...
            foreach ($_POST['student_id'] as $key => $row) {
                


                   $data['item_id'] = $value;
                   $data['student_id'] = $row;
                   $data['marks'] = $_POST['marks'][$i];
                   $data['exam_id'] = $_POST['exam_id'];
                   $data['class_id'] = $_POST['class_id']; 
                   $data['section_id'] = $_POST['section_id'];
                   $data['subject_id'] = $_POST['subject_id'];
                   $data['case_title_id'] = $_POST['case_title_id'];
                   $data['year'] = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description; 

                   $this->db->insert('tbl_term_case_marks',$data);              
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
        redirect(base_url() . 'index.php?admin/set_case_marks_view');
    ...
    function set_case_marks()
    {
        ...
        {           
               ...
               $page_data['case_title_id'] = $_POST['case_title_id'];
                ...
                $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
               $case_title = $this->db->get_where('tbl_case',array('class_id' => $page_data['class_id'], 'exam_id' => $page_data['exam_id'], 'year' => $year, 'case_title_id' => $page_data['class_id']))->row();

               $page_data['student_details'] = $this->db->select('*')->from('enroll')->where('class_id',$_POST['class_id'])->where('section_id',$_POST['section_id'])->where('year',$year)->get()->result();

                $page_data['case_items'] = $this->db->select('*')->from('tbl_caa_item')->where('year',$year)->get();


        $page_data['page_title']    = get_phrase('add_case_marks');
        $page_data['page_name']  = 'add_case_marks';
    ...
    function getting_case_title_id()
    {
        $exam_id = $_POST['exam_id'];
        $subject_id = $_POST['subject_id'];

        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $query =  $this->db->select('*')->from('tbl_case')->where('exam_id',$exam_id)->where('year',$year)->where('subject_id',$subject_id)->get()->row();



        $case_title_name = $this->db->select('*')->from('exam')->where('exam_id',$query->case_title_id)->where('year',$year)->get()->row();
            
            if($case_title_name != '')
            {

                   echo '<option value='.$case_title_name->exam_id.'>'.$case_title_name->name.'</option>';       
            } else {
   ...
   echo '<option>no data found</option>';       
    ...
    function subject($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            if($this->input->post('type') == 'theory')
            {

                $data['name'] = $this->input->post('name').' '.'theory';
            ...
            if($this->input->post('type') == 'th/pr')
                
                {
                    $data['name'] = $this->input->post('name').' '.'theory';
                    $data1['name'] = $this->input->post('name').' '.'practicle';
    ...
    function classes($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'create') {
            $data['name']         = $this->input->post('name');
            $data['teacher_id']   = $this->input->post('teacher_id');
            $data['year'] = $running_year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
    ...
    function Update_student_section()
    {
        ...
        $page_data['page_name']  = 'assign_student_section';
        $page_data['page_title'] = get_phrase('Update_student_section');
        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $page_data['class'] = $this->db->get_where('class',array('year' => $year))->result();
        $this->load->view('backend/index', $page_data);

    }

    // get_student_section_update_section
    ...
    function get_student_classwise($class_id)
    {
        ...
        $query = $this->db->select('*')->from('enroll')->where('class_id',$class_id)->where('year',$year)->get()->result();
        ...
        foreach ($query as $key => $value) {
            ...
            echo"<td><input type='checkbox' name='student_id[]' id='student_id".++$key."' class='std_id'  value=".$value->student_id."></td>";
            echo '<td>'.$value->year.'</td>';
            echo '<td>'.$value->joining_date.' </td>';
            echo '<td><a href="#" id ="std_name" onClick="check_man('.$value->student_id.')">'.$this->db->select('*')->from('student')->where('student_id',$value->student_id)->get()->row()->name.'</a></td>';
            echo '<td>'.$this->db->get_where('section',array('section_id' => $value->section_id))->row()->name.'</td>';
    ...
    // store student_classwise

    function store_students_section_info()
    {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST")
        {
            ...
            foreach ($_POST['std_id'] as $key => $value) {
              ...
              $this->session->set_flashdata('flash_message' , get_phrase('data_deleted'));
      redirect(base_url() . 'index.php?admin/update_student_section' , 'refresh');
            ...
            redirect(base_url() . 'index.php?admin/update_student_section/');
        }

    }

    // get student details for update rollno when chnge value on class

    function get_student_rollno_classwise($class_id)
    {
       ...
       $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        ...
        foreach ($query as $key => $value) {
            ...
            echo '<td>'.$value->year.'</td>';
            echo '<td>'.$value->joining_date.' </td>';
            echo '<td><a href="#" onClick="check_man('.$value->student_id.')">'.$this->db->select('*')->from('student')->where('student_id',$value->student_id)->get()->row()->name.'</a></td>';

            if ($value->roll != '') {
              
               echo"<td><input type='text' name='roll' value=".$value->roll." class='form-control'></td>";         

            } else {

                echo '<td>'.ucfirst("not assign roll").' </td>';
            }
    ...
    // get student details for update roll no when change value on section

    function get_student_rollno_sectionwise($class_id,$section_id)
    {

        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;

        $query = $this->db->select('*')->from('enroll')->where('class_id',$class_id)->where('section_id',$section_id)->where('year',$year)->get()->result();        
    ...
    function update_rollno()
    {

        $page_data['page_name']  = 'update_rollno';
        $page_data['page_title'] = get_phrase('update_rollno');
        $year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
        $page_data['class'] = $this->db->get_where('class',array('year' => $year))->result(); 
    ...
    function exam($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            $data['mark_distribution']    = $this->input->post('mark_distribution');
            $data['year']    = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
    ...
    function subject_credit($param1= '')
    {

        if($param1 == 'update_subject_credit')
        {
        ...
        $running_year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
        $page_data['class'] = $this->db->get('class')->result();
        $page_data['page_title'] = get_phrase('subject_credit');
        $page_data['page_name'] = 'subject_credit';
        $page_data['running_year'] = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;    
    ...
    function routine($param1 = '')
    {
        ...
        if($param1 == 'add')
        {
            ...
            $data['subject_id'] = $this->input->post('subject_id');
            $data['start_date'] = $this->input->post('start_date');
            $data['start_from'] = $this->input->post('start_time');
            $data['end_date'] = $this->input->post('end_time');
            $data['comment'] = $this->input->post('comment');
            $data['max_num'] = $this->input->post('full_mark');
            $data['min_num'] = $this->input->post('pass_mark');
            $data['academic_year'] = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;;
        ...
        $running_year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
        $page_data['class'] = $this->db->get('class')->result();
        $page_data['page_title'] = get_phrase('subject_routine');
        $page_data['exam'] = $this->db->select('*')->from('exam')->where('year',$running_year)->get()->result_array();
        $page_data['routine'] = $this->db->get_where('tbl_routine',array('academic_year' => $running_year))->result_array();
        $page_data['page_name'] = 'routine';
        $page_data['running_year'] = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;    
    ...
    function get_subject_details($class_id)
    {

        $subject = $this->db->get_where('subject',array('class_id' => $class_id))->result();

          echo '<option value="">'.get_phrase('select_subject').'</option>';

          foreach ($subject as $key => $value) {

            echo '<option value='.$value->subject_id.'>'.$value->name.'</option>';            

                    }          

    }

    // get subject id for case

    function get_subject_id_case($class_id)
    {

         $running_year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description; 
        $query = $this->db->get_where('subject',array('class_id' => $class_id, 'year' => $running_year))->result();
    ...
    function get_subject_id($class_id)
    {
        ...
        $running_year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description; 
        $query = $this->db->select('*')->from('subject')->where('class_id',$id)->where('year',$running_year)->get()->result();

        // echo '<option value="">'.get_phrase('select_subject').' </option>';

        echo '<td style="text-align: center;">
     "'.get_phrase('students').'" <i class="entypo-down-thin"></i>|"'.get_phrase('subject').'"  <i class="entypo-right-thin"></i></td>';
    ...
    function assessment($param1 = '')
    {
        ...
        if($param1 == 'store')
        {
            ...
            $this->db->insert('tbl_assesment',$data);
        ...
        $page_data['running_year'] = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
        $page_data['assessment'] = $this->db->get_where('tbl_assesment', array('year' => $page_data['running_year']))->result_array();
...
// get section _id for show marks of totle student

    function getting_section_id($class_id)
    {
    ...
    function marksheet()
    {
        ...
        $year = $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;
        $data['marks'] = $this->db->get('tbl_term_marks')->result();
        $data['class'] = $this->db->get('class')->result();
        $data['enroll'] = $this->db->select('*')->from('enroll')->where('year',$year)->get()->result();


        $data['page_name']  = 'marksheet1';
        $data['page_title'] = get_phrase('marksheet');

        $this->load->view('backend/index', $data); 

    }

    function grade_system()
    {
    ...
    function grade_list()
    {
        ...
        $data['marks_term'] = $this->db->select('*')->from('tbl_term_marks')->where('class_id',$data['class_id'])->where('section_id',$data['section_id'])->where('exam_id', $data['exam_id'])->get()->result();

        $year = $this->db->get_where('settings',array('type' => 'running_year'))->row()->description;
        $data['page_name']  = 'marksheet2';
        $data['page_title'] = get_phrase('marksheet');

        $data['subjects'] = $this->db->select('*')->from('tbl_routine')->where('academic_year',$year)->where('exam_id',$data['exam_id'])->where('class_id',$data['class_id'])->get();
        
        $data['students'] = $this->db->select('*')->from('enroll')->where('year',$year)->where('class_id',$data['class_id'])->where('section_id',$data['section_id'])->get()->result();
    ...
    function calculate_marksheet()
    {

        $data['marks'] = $this->db->get('tbl_term_marks')->result();
        $data['exam'] = $this->db->get('exam')->result();
        $data['page_name']  = 'marksheet';
        $data['page_title'] = get_phrase('marksheet');
        $this->load->view('backend/index', $data); 
    }
    ...
    function students_marksheet()
    {
           ...
           $section_id = $_POST['section_id'];
           $exam_id = $_POST['exam_id'];
           $running_year = $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
           $student_details = $this->db->select('*')->from('enroll')->where('class_id',$class_id)->where('year',$running_year)->where('section_id',$section_id)->get()->result();


           echo "<form action='javascript:void(0)'>";

           foreach ($student_details as $key => $value) {
                $std_name = $this->db->get_where('student',array('student_id' => $value->student_id))->row();
                $std_term_marks = $this->db->select('*')->from('tbl_term_marks')->where('class_id',$class_id)->where('student_id',$value->student_id)->where('exam_id',$exam_id)->where('section_id',$section_id)->get()->result();


                // foreach ($std_name as $key => $value) {
                    

                
                // echo "<pre>";
                //    print_r($std_term_marks);exit(); 
                ...
                echo "
                    ...
                    <td>$std_name->name</td>
                    <td>$value->enroll_code</td>
                    <td>

                    <button type='submit' onClick='check_man(".$std_name->student_id.")' data-toggle='modal' data-target='#myModal'  class='btn btn-sm btn-success'>".get_phrase('Report')."</a>
    ...
    function grade_marks($param1='', $param2='')
    {

        if($param1 == 'store')
        {
            
            $this->db->insert('grade_marks',$_POST);
        ...
        $page_data['exams'] = $this->db->get_where('exam', array('year' => $running_year))->result_array();
        $page_data['grade'] = $this->db->get('grade_marks')->result_array();
        $page_data['page_name']  = 'grade_marks';
        $page_data['page_title'] = get_phrase('grade_scale');
        $this->load->view('backend/index', $page_data);      
    ...
    function store_student_marks()
    {
        ...
        foreach ($_POST['subject_id'] as $key => $value) {
            ...
            foreach ($_POST['student_id'] as $key => $row) {
                   ...
                   $this->db->insert('tbl_term_marks',$data);   
        ...
        redirect(base_url('index.php?admin/set_term_marks'));
    }
    ...
    function select_case()
    {
        ...
        $page_data['page_title']    = get_phrase('select_case_requirement');
        $page_data['page_name']     = 'filter_case';
    ...
    function get_case_id()
    {
        ...
        // $query = $this->db->get_where('tbl_case',array('class_id' => $class_id, 'section_id' => $section_id, 'exam_id' => $exam_id, 'subject_id' => $subject_id))->row();
        $query = $this->db->select('*')->from('tbl_case')->where('class_id',$class_id)->where('exam_id', $exam_id)->where('year',$running_year)->get()->row()->case_title_id;

        $value = $this->db->get_where('exam',array('exam_id' => $query))->row();
    ...
    function case_marks_entry()
    {
        ...
        $running_year =  $this->db->get_where('settings' , array(
                 'type' => 'running_year'))->row()->description;
        $page_data['students'] = $this->db->select('*')->from('enroll')->where('class_id',$class_id)->where('section_id',$section_id)->where('year',$running_year)->get()->result();
        // $page_data['page_title']    = get_phrase('select_case_requirement');
        // $page_data['page_name']  = 'case_marks_entry';
        $page_data['exam_id'] = $_POST['exam_id'];
        $page_data['class_id'] = $_POST['class_id'];
        $page_data['section_id'] = $_POST['class_id'];
        $page_data['subject_id'] = $_POST['subject_id'];
        $page_data['res'] = $this->db->select('*')->from('tbl_term_case_marks')->where('year',$running_year)->where('class_id',$page_data['class_id'])->where('section_id',$page_data['section_id'])->where('exam_id',$page_data['exam_id'])->get()->result();

        // print_r($page_data['res']);exit();

        $page_data['students'] = $this->db->select('*')->from('enroll')->where('class_id',$class_id)->where('section_id',$section_id)->where('year',$running_year)->get()->result();

        $this->load->view('backend/admin/case_marks_entry', $page_data);          

    }

    function case_marks_added()
    {
        ...
        if($_SERVER['REQUEST_METHOD'] == "POST")
        {
           ...
           $i = 0;
            ...
            foreach ($_POST['std_id'] as $key => $row) {
                


                   $data['student_id'] = $row;
                   $data['classwork'] = $_POST['classwork'][$i];
                   $data['behaviour'] = $_POST['behaviour'][$i];
                   $data['creativity'] = $_POST['creativity'][$i];
                   $data['homework'] = $_POST['homework'][$i];
                   $data['attendence'] = $_POST['attendence'][$i];
                   $data['class_id'] = $_POST['class_id']; 
                   $data['section_id'] = $_POST['section_id'];
                   $data['case_id'] = $_POST['case_title_id'];
                   $data['exam_id'] = $_POST['exam_id'];
                   $data['subject_id'] = $_POST['subject_id']; 
        ...
        $this->session->set_flashdata('flash_message' , get_phrase('data_updated'));
                ...
                redirect(base_url('index.php?admin/select_case'));   
    ...
    
    {
        ...
        foreach ($_POST['std_id'] as $key => $value) {
            ...
            $this->db->where('student_id',$value);  
            $this->db->update('tbl_term_case_marks',$data);
